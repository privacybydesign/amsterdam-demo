...
describe('QR Code', () => {
    it('should render the initial QR Code button, and not the modal', async () => {
        // Render QR Code
        await act(async (): Promise<any> => await wrappedRender(<QRCode label="Test" getSession={getSession} />));

        const QRCodeButton: HTMLElement = await screen.findByTestId('qrCodeButton');
        expect(QRCodeButton).toBeInTheDocument();

        const QRCodeModal: HTMLElement | null = await screen.queryByTestId('qrCodeModal');
